...
如果你修改的是 release gate、公开文档、部署拓扑或入口分层，建议至少补跑：

```bash
python3 scripts/bootstrap_env.py --check
uv run python scripts/run_verification_smoke.py
uv run python -m pytest -q tests/test_release_management_suite.py tests/test_architecture_closure_assets.py
```
...
- 治理规则
- GitHub 对外表述
- onboarding / handoff 路径
- release readiness / active sign-off 清单
