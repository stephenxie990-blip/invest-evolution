...
def register_runtime_ops_routes(
    ...
    *,
    ...
    def api_data_status():
        try:
            view = parse_view_arg(request.args.get("view", "json"))
            refresh = parse_bool(request.args.get("refresh", False), "refresh")
        except ValueError as exc:
            return jsonify({"error": str(exc)}), 400
        runtime = get_runtime()
        if runtime is not None:
            return respond_with_display(runtime.get_data_status(refresh=refresh), view=view)
        return respond_with_display(get_data_status_payload(refresh=refresh), view=view)
