...
> 这个系统的目标用户首先是 Agent，而不是人类操作员。  
> 人类在这里更像治理者、授权者、观察者和纠偏者；Agent 才是围绕数据底座和任务协议工作的第一执行单元。

当前项目里的 Agent 主要分成四层：市场判断、选股猎手、复盘裁判、系统编排。

## 0. Agent-first 协作哲学

- 系统先为 Agent 提供稳定上下文、结构化输入、有限行动空间和可审计输出。
- 人类界面主要承担控制面职责：查看状态、调整配置、批准变更、排查异常。
- Agent 不是“自由发挥的参与者”，而是被 prompt、代码边界、工具面和治理策略共同约束的工具。
- 角色分工的目的不是叙事感，而是把“市场判断 / 候选生成 / 复盘裁决 / 系统编排”拆成更容易观察与验证的协作单元。
...
- **JSON-only 输出约束**：让结果更适合被程序消费，而不是停留在自然语言叙述。
- **role-bounded 输出约束**：防止 MarketRegime 去改参数、Hunter 去越权做组合决策、Commander 去伪装领域专家。
