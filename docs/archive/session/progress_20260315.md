...
### Governance rerun closeout

- 读取并采用技能：`pi-planning-with-files`、`python-patterns`、`python-testing`、`tdd-workflow`、`verification-loop`
- 完成两项同步目标：
  - 基于新治理基线完成 `20-cycle` 训练 rerun
  - 将 `promotion_gate / freeze_gate` 默认策略参数统一上收到配置层

### Completed

- 新增 `invest/shared/model_governance.py`
  - 定义 `DEFAULT_FREEZE_GATE_POLICY`
  - 新增 `normalize_freeze_gate_policy(...)`
  - 统一 `quality_gate_matrix / promotion_gate / freeze_gate` 默认治理策略面
- 更新 `app/training/policy_services.py`
  - `sync_runtime_policy()` 现会对 `train.freeze_gate` 做默认归一化注入
- 更新 `app/training/reporting.py`
  - `evaluate_freeze_gate()` / `build_freeze_report()` 现统一基于归一化后的 freeze gate 运行
- 更新测试：
  - `tests/test_training_controller_services.py`
  - `tests/test_research_training_feedback.py`
- 完成完整 rerun：
  - 输出目录：`outputs/phase_v11_validation_20260315_governance_rerun`
  - 真实训练落盘到 `cycle_20.json`
  - `optimization_events.jsonl`、meeting logs、research artifacts、config snapshots 均已生成
- 完成正式对比文档：
  - `docs/audits/MODEL_GOVERNANCE_RERUN_COMPARISON_20260315.md`

### Results

- 治理修复验证通过：
  - 新 rerun 的 `optimization_events.jsonl` 共 `44` 条事件，全部通过 `contract_check`
  - review / research_feedback / yaml_mutation 已统一纳入事件审计链
  - routing/allocator/leaderboard 质量门已生效，`eligible_models=0`
  - `candidate_generated -> candidate_pending -> candidate_pruned` 真实出现
- 效果面未通过生产门：
  - 最佳条目 `score=-15.640334`
  - `avg_return_pct=-1.726543`
  - `avg_sharpe_ratio=-2.412323`
  - `eligible_for_routing=false`
  - `promotion_gate_status=rejected`
  - research feedback gate 明确失败，freeze 不成立

### Residual risks

- run-specific `leaderboard.json` 停在 `latest_cycle_id=19`，与 `cycle_20.json` 不完全同步
- 运行工件中 `policy.train.promotion_gate / freeze_gate` 仍未显式回写 resolved defaults，审计面还可继续加强

### Verification

- `.venv/bin/ruff check app/train.py app/training/controller_services.py app/training/policy_services.py app/training/reporting.py app/lab/artifacts.py invest/shared/model_governance.py tests/test_training_controller_services.py tests/test_lab_artifacts.py tests/test_commander.py tests/test_research_training_feedback.py` -> pass
- `.venv/bin/pyright app/train.py app/training/controller_services.py app/training/policy_services.py app/training/reporting.py app/lab/artifacts.py invest/shared/model_governance.py tests/test_training_controller_services.py tests/test_research_training_feedback.py` -> `0 errors, 0 warnings`
- `.venv/bin/pytest tests/test_training_controller_services.py tests/test_lab_artifacts.py tests/test_commander.py tests/test_research_training_feedback.py -q` -> pass
...
- 读取并采用技能：`pi-planning-with-files`、`python-patterns`、`python-testing`、`tdd-workflow`、`verification-loop`
- 复核 `docs/plans/MODEL_OPTIMIZATION_REMEDIATION_BLUEPRINT_20260315.md`、`task_plan.md`、`findings.md`
- 将本轮目标固定为按顺序完成：优化审计、路由准入、候选晋级纪律、质量门矩阵、治理后效果优化、Agent 证据化

### Completed

- 新增 `invest/shared/model_governance.py`
- `app/train.py` 中 `OptimizationEvent` 已补 `cycle_id / event_id / lineage / evidence / contract_version`
- `app/training/optimization.py` 已为 optimization event 写入 lineage/evidence，并覆盖 `yaml_mutation / llm_analysis / research_feedback / evolution_engine / error`
- `app/training/review_stage_services.py` 已让 review event 落盘审计，并与 structured lineage/evidence 对齐
- `app/training/experiment_protocol.py`、`app/training/lineage_services.py`、`app/training/promotion_services.py` 已引入 `active / candidate / override` 阶段和 promotion discipline
- `invest/leaderboard/engine.py`、`invest/allocator/engine.py`、`invest/router/engine.py` 已接入质量门；正式路由不再吃未过门候选
- 新增/更新测试：
...
- `.venv/bin/pytest tests/test_training_optimization.py tests/test_training_promotion_lineage.py tests/test_training_experiment_protocol.py tests/test_model_governance.py tests/test_leaderboard.py tests/test_allocator.py tests/test_model_routing.py -q` -> pass
- `.venv/bin/pytest tests/test_training_controller_services.py tests/test_research_training_feedback.py -q` -> pass
- `ruff check app/train.py app/training/optimization.py app/training/review_stage_services.py app/training/experiment_protocol.py app/training/lineage_services.py app/training/promotion_services.py app/training/reporting.py app/training/policy_services.py invest/shared/model_governance.py invest/leaderboard/engine.py invest/allocator/engine.py invest/router/engine.py tests/test_training_optimization.py tests/test_training_promotion_lineage.py tests/test_training_experiment_protocol.py tests/test_model_governance.py tests/test_leaderboard.py tests/test_allocator.py tests/test_model_routing.py` -> pass
- `.venv/bin/pyright app/train.py app/training/optimization.py app/training/review_stage_services.py app/training/experiment_protocol.py app/training/lineage_services.py app/training/promotion_services.py app/training/reporting.py app/training/policy_services.py invest/shared/model_governance.py invest/leaderboard/engine.py invest/allocator/engine.py invest/router/engine.py` -> `0 errors`
...
### Model optimization remediation planning

- 新增 `docs/plans/MODEL_OPTIMIZATION_REMEDIATION_BLUEPRINT_20260315.md`
- 明确下一阶段不再做大规模结构重构，而是聚焦模型治理闭环
- 将下一阶段拆为 6 个工作流：
  - 优化审计修复
  - 路由准入收紧
  - 候选晋级纪律
  ...
  - Agent 证据化与测试加固
- 规划结论：
  - 先修 `OptimizationEvent`
  - 再修 `leaderboard / allocator / router`
  - 再修 `promotion / lineage`
  - 最后 rerun 验证

### Verification

- 本轮仅新增规划文档与 planning files 更新，未修改业务代码

## 2026-03-15 governance rerun in progress

### Config-layer governance completion

- 已将 `freeze_gate` 默认策略与 `promotion_gate` 一样收敛到统一配置层语义：
  - 新增 `DEFAULT_FREEZE_GATE_POLICY`
  - 新增 `normalize_freeze_gate_policy(...)`
  - `TrainingPolicyService.sync_runtime_policy()` 现在会对 `train.freeze_gate` 做默认模板 + YAML 覆盖归一化
  - `build_freeze_report()` / `evaluate_freeze_gate()` 会基于归一化后的策略生成最终门槛与评估结果
- 已补回归测试：
  - `tests/test_training_controller_services.py`
  - `tests/test_research_training_feedback.py`
- 已完成 focused verification：
  - `.venv/bin/ruff check ...` -> pass
  - `.venv/bin/pyright ...` -> `0 errors`
  - `.venv/bin/pytest tests/test_training_controller_services.py tests/test_lab_artifacts.py tests/test_commander.py tests/test_research_training_feedback.py -q` -> pass
...
- 已启动完整复跑：
  - `source .venv/bin/activate && python train.py --cycles 20 --force-full-cycles --output outputs/phase_v11_validation_20260315_governance_rerun --meeting-log-dir outputs/phase_v11_validation_20260315_governance_rerun/meetings --config-audit-log-path outputs/phase_v11_validation_20260315_governance_rerun/config_changes.jsonl --config-snapshot-dir outputs/phase_v11_validation_20260315_governance_rerun/config_snapshots`
- 复跑过程中已确认的新工件事实：
  - `leaderboard.json` 现在显式暴露 `quality_gate_matrix`
  - `leaderboard.entries[*]` 新增 `eligible_for_routing / deployment_stage / promotion_status / quality_gate / objective_eligible_after_governance`
  - `optimization_events.jsonl` 中真实运行事件已包含：
    - `event_id`
...
- 已观察到的真实治理行为：
  - review 事件直接落入 `optimization_events.jsonl`
  - `research_feedback -> yaml_mutation` 形成真实 `candidate_generated` 证据链
  - `candidate_pending` 已在真实运行中演进出 `candidate_pruned`
  - routing 资格被持续收紧，`eligible_models` 在中程仍为 `0`
  - 即便阶段性单轮收益转正，只要 `score / avg_return / deployment_stage` 不过线，仍不会进入正式路由
- 当前中途状态（以运行过程中的 `leaderboard.json` 为准）：
  - 已落盘 `10` 个真实周期中的前半段快照
  - 截至 `10` 周期快照，`momentum` 仍 `eligible_for_routing=false`
  - 截至 `10` 周期快照，`promotion_status` 已出现 `candidate_pruned`
...
- 复跑结束后执行：
  - 新旧 run 结果对比抽取
  - `docs/audits/MODEL_GOVERNANCE_RERUN_COMPARISON_20260315.md` 填充正式结论
  - planning files 最终收口
