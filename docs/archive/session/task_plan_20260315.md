...
## 2026-03-15 governance closeout

| Workstream | Status | Outcome | Notes |
|---|---|---|---|
| Optimization audit repair | complete | `OptimizationEvent` 已补 `cycle_id / lineage / contract_check` 并进入真实 rerun 工件 | 已由 rerun 证据验证 |
| Routing admission tightening | complete | leaderboard / allocator / router 已共享质量门 | `eligible_models=0`，未出现误路由 |
| Candidate promotion discipline | complete | `active / candidate / override` 已分离，并形成 `pending / pruned` 纪律轨迹 | 真实运行已出现多次 `candidate_pruned` |
| Promotion / freeze policy surfacing | complete | `promotion_gate / freeze_gate` 默认策略已上收到共享配置层 | 测试守卫通过 |
| Governance rerun | complete | `20-cycle` 复跑已完成，正式对比报告已落盘 | `docs/audits/MODEL_GOVERNANCE_RERUN_COMPARISON_20260315.md` |
| Effect optimization | pending | 下一阶段在治理基线稳定后推进 | 不回退治理门，只做质量优化 |

## Current decision

- 当前阶段判定为：治理修复闭环已完成，可以进入效果优化阶段。
- 下一阶段优先级：
  1. 修复最终工件一致性，确保 `cycle_20` 后强制刷新 leaderboard
  2. 将 resolved gate policy 显式写入最终工件
  3. 针对 bull regime、agent 证据对齐、benchmark/Sharpe/回撤目标做专项优化
...
## 2026-03-15 Model Governance Remediation

### Goal

- 按顺序完成模型治理修复：优化审计、路由准入、候选晋级纪律、训练质量门矩阵、治理后效果优化、Agent 证据化与测试加固。

### Execution status

| Step | Status | Scope | Notes |
|---|---|---|---|
| 1 | complete | 优化审计修复 | `OptimizationEvent` 已补 `cycle_id / lineage / evidence / contract`，review event 也会落盘审计 |
| 2 | complete | 路由准入收紧 | leaderboard / allocator / router 已引入正式质量门，负分或非 active 候选不得进入正式路由 |
| 3 | complete | 候选晋级纪律 | 已区分 `active / candidate / override`，并建立 pending / pruned / expired 纪律状态 |
| 4 | complete | 训练质量门矩阵 | 新增共享治理矩阵，统一 review / optimization / routing / promotion / freeze 参考标准 |
| 5 | complete | 效果提升回路 | leaderboard 已显式区分 `objective_profile` 与 `objective_eligible_after_governance` |
| 6 | complete | Agent 证据化与测试加固 | reasoning 与 evidence 对齐，补齐 governance 契约与主链路回归测试 |
...
- `ruff check ...`（本轮改动文件） -> pass
- `.venv/bin/pyright ...`（本轮改动文件） -> 0 errors
...
## 2026-03-15 Model optimization remediation planning

### Goal

- 在不重启大规模结构重构的前提下，为“模型优化与修复”建立可直接执行的蓝图和阶段实施路径。
...
- 优先顺序固定为：
  - 优化事件审计修复
  - 路由准入质量门
  - candidate / active / override 晋级纪律
  - 统一质量门矩阵
  - rerun 验证与效果提升

### Delivered

- 已新增 `docs/plans/MODEL_OPTIMIZATION_REMEDIATION_BLUEPRINT_20260315.md`
- 已将下一阶段工作流明确拆为：
  - 工作流 A：优化审计修复
  - 工作流 B：路由准入收紧
  - 工作流 C：候选晋级纪律
  - 工作流 D：训练质量门矩阵
  - 工作流 E：效果提升回路
  - 工作流 F：Agent 证据化与测试加固

### Next execution order

1. 修 `OptimizationEvent` 和 optimization lineage
2. 修 `leaderboard / allocator / router` 的质量门
3. 修 `promotion_services / lineage_services` 的 candidate discipline
4. 跑 focused regression
5. 跑新一轮 20-cycle 验证并对比基线

## 2026-03-15 governance rerun closeout plan

### Goal

- 同步完成两件事：
  - 基于新治理基线跑一轮完整 `20-cycle` 训练复跑
  - 将 `promotion gate / freeze gate` 默认策略参数正式上收到配置层并形成可调治理策略面

### Current status

- 配置层上收：`in_progress`
- 完整 rerun：`in_progress`
- 对比报告：`pending`

### Execution checklist

1. 完成 `freeze_gate` 与 `promotion_gate` 的同构默认策略归一化
2. 完成 focused verification
3. 跑 `outputs/phase_v11_validation_20260315_governance_rerun`
4. 对比基线与复跑的：
   - `leaderboard`
   - `optimization_events`
   - `promotion/lineage/governance_metrics`
   - `freeze_gate_evaluation`
   - `eligible_for_routing`
5. 输出正式对比文档并更新 planning files

### Interim checkpoints

- 已完成 checkpoint A：
  - `freeze_gate` 默认策略归一化与测试回归通过
- 已完成 checkpoint B：
  - 新 rerun 工件已证明事件契约、lineage、quality gate 已进入真实输出
- 正在执行 checkpoint C：
  - 等待 `20-cycle` 全量复跑完成后收集最终指标
