...
1. 入口层：`commander.py`、`train.py`、`web_server.py`
...
- `commander.py`：统一 CLI / Daemon / Runtime 装配入口
- `train.py`：训练/研究专用入口
- `web_server.py`：Flask API / 前端入口
...
- `commander.py`：负责把 `brain/` 与 `invest/` 统一装配成一个运行时
