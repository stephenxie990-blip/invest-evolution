...
class SelectionMeeting:
    ...
    def _run_llm(
        ...
        if self._debate is not None and stock_summaries:
            self._notify_progress({"agent": "SelectionMeeting", "status": "running", "stage": "debate", "progress_pct": 52, "message": "多空辩论筛选候选中..."})
            ...
            self._notify_progress({
                ...
                "details": [{"code": code, **result} for code, result in list(debate_results.items())[:10]],
            })
