...
def _fallback_sections(body: dict[str, Any], summary: str) -> tuple[list[dict[str, Any]], list[str], str]:
    ...
    if config:
        ...
        default_manager_id = str(
            config.get("default_manager_id")
            or body.get("dominant_manager_id")
            or ""
        ).strip()
        if default_manager_id:
            facts.append(f"默认经理：{default_manager_id}")
    ...
    best_entry = body.get("best_entry")
    if isinstance(best_entry, dict):
        best_name = str(
            best_entry.get("manager_id")
            or best_entry.get("manager_config_ref")
            or ""
        ).strip()
        if best_name:
            facts.append(f"当前最佳经理：{best_name}")
    ...
    if latest_result:
        ...
        if ops_panel.get("available", False):
            ...
            if refs.get("active_runtime_config_ref"):
                ops_items.append(f"活动配置：{refs.get('active_runtime_config_ref')}")
            if refs.get("candidate_runtime_config_ref"):
                ops_items.append(f"候选配置：{refs.get('candidate_runtime_config_ref')}")
            if refs.get("candidate_runtime_config_meta_ref"):
                ops_items.append(f"候选元数据：{refs.get('candidate_runtime_config_meta_ref')}")
            if review_window:
                ops_items.append(
