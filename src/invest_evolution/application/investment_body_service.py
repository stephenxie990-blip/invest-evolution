...
from invest_evolution.application.commander.workflow import jsonable as _jsonable
...
from invest_evolution.market_data import DataManager, DataSourceUnavailableError, MockDataProvider

if TYPE_CHECKING:
    from invest_evolution.application.commander_main import CommanderConfig
...
class InvestmentBodyService:
    ...
    def __init__(
        ...
        self.controller = SelfLearningController(
            ...
            artifact_log_dir=str(self.cfg.artifact_log_dir),
        ...
        return _jsonable(
            {
                ...
                "execution_defaults": {
                    "default_manager_id": str(getattr(config, "default_manager_id", "momentum") or "momentum"),
                ...
                "governance_enabled": getattr(self.controller, "governance_enabled", False),
                "governance_mode": getattr(self.controller, "governance_mode", "off"),
                "last_governance_decision": governance_from_controller(self.controller),
    ...
    def _artifact_paths_for_cycle(self, cycle_id: int | None) -> dict[str, str]:
        ...
        return {
            ...
            "selection_artifact_json_path": str(
                self.cfg.artifact_log_dir / "selection" / f"selection_{cid:04d}.json"
            ),
            "selection_artifact_markdown_path": str(
                self.cfg.artifact_log_dir / "selection" / f"selection_{cid:04d}.md"
            ),
            "manager_review_artifact_json_path": str(
                self.cfg.artifact_log_dir / "manager_review" / f"manager_review_{cid:04d}.json"
            ),
            "manager_review_artifact_markdown_path": str(
                self.cfg.artifact_log_dir / "manager_review" / f"manager_review_{cid:04d}.md"
            ),
            "allocation_review_artifact_json_path": str(
                self.cfg.artifact_log_dir / "allocation_review" / f"allocation_review_{cid:04d}.json"
            ),
            "allocation_review_artifact_markdown_path": str(
                self.cfg.artifact_log_dir / "allocation_review" / f"allocation_review_{cid:04d}.md"
            ),
        ...
        return _jsonable(
            {
                ...
                "execution_defaults": execution_defaults_payload(
                    normalize_governance_decision(getattr(result, "governance_decision", {})),
                ...
                "governance_decision": normalize_governance_decision(getattr(result, "governance_decision", {})),
                ...
                "research_artifacts": result.research_artifacts,
                "ab_comparison": result.ab_comparison,
                "experiment_spec": result.experiment_spec,
                ...
                "causal_diagnosis": result.causal_diagnosis,
                "similarity_summary": result.similarity_summary,
                "similar_results": result.similar_results,
                "realism_metrics": result.realism_metrics,
