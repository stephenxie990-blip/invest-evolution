...
from invest_evolution.investment.foundation.compute import calc_rsi
from invest_evolution.investment.shared.contracts import TradingPlan
...
from .risk import EmergencyDetector, RiskController
...
class SimulatedTrader:
    ...
    def get_price(self, ts_code: str, date: str) -> Optional[float]:
        ...
        if not row.empty:
            return _normalize_price_value(row.iloc[0]["close"])
        df_b = df[df["trade_date"] < date]
        if not df_b.empty:
            return _normalize_price_value(df_b.iloc[-1]["close"])
        return None
    ...
    def buy(
        ...
        if (
            not math.isfinite(buy_price)
            or buy_price <= 0
            or not math.isfinite(self.cash)
            or self.cash <= 0
        ):
