...
from typing import Any, Dict, Iterable, Mapping, Protocol, Sequence, cast
...
class WebDatasetService:
    ...
    def get_status_summary(self, *, refresh: bool = False) -> dict[str, Any]:
        from .manager import DataQualityService
...
def _stale_training_data(*, latest_date: str | None, cutoff_date: str) -> bool:
    normalized_latest = str(latest_date or "").strip()
    if not normalized_latest:
        return False
    return normalize_date(normalized_latest) < normalize_date(cutoff_date)


def _training_readiness_contract(
    *,
    ...
    stale_data = _stale_training_data(latest_date=latest_date, cutoff_date=cutoff_date)
    ready = bool(has_kline_data and eligible_count > 0 and not stale_data)
    return {
        "ready": ready,
        "stale_data": stale_data,
        "requires_explicit_override": stale_data,
        "severity": "blocking" if not ready else "ok",
    }
        ...
        df = pd.DataFrame(
            {
                ...
                "close": close,
                "volume": np.random.randint(5_000_000, 50_000_000, size=days),
                "amount": close * np.random.randint(5_000_000, 50_000_000, size=days),
                "pct_chg": pd.Series(close).pct_change().fillna(0) * 100,
                "turnover": np.random.uniform(0.5, 12.0, size=days),
                "code": code,
            }
        )
        stock_data[code] = df
...
class DataSourceUnavailableError(RuntimeError):
    ...
    def __init__(
        ...
        *,
        ...
        offline_diagnostics: dict[str, Any] | None = None,
        online_error: str = "",
        ...
        if str(source or "").strip():
            offline_payload.setdefault("source", str(source).strip())
...
class EvolutionDataLoader:
    """Online fallback loader used only when offline data is unavailable."""
    ...
    def load_all_data_before(self, cutoff_date: str) -> dict[str, Any]:
        ...
        try:
            for code in self.stock_pool:
                ...
                if getattr(rs, "error_code", "0") != "0" or rs is None:
                    continue
