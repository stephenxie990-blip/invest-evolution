...
def join_financials_point_in_time(
    ...
    """Attach financial snapshots by code/trade_date point-in-time as-of join."""
    if bars.empty:
    ...
    left["_trade_dt"] = pd.to_datetime(
        left["trade_date"].astype(str).map(normalize_date), format="%Y%m%d", errors="coerce"
    )
    ...
    right["_effective_dt"] = pd.to_datetime(
        right["effective_date"].astype(str), format="%Y%m%d", errors="coerce"
    )
