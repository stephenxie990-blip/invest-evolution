...
from typing import Any, Callable, Iterable, Iterator, Mapping, Optional, Protocol, Sequence, cast
...
class MarketDataRepository:
    ...
    def _configure_connection_pragmas(self, conn: sqlite3.Connection) -> None:
        try:
            ...
            logger.warning(
                "Falling back to DELETE journal mode for %s after WAL setup failed: %s",
                self.db_path,
                exc,
            )
    ...
    def connect(self) -> Iterator[sqlite3.Connection]:
        ...
        try:
            ...
            if conn is not None:
                conn.rollback()
            logger.exception("Database operation failed: %s", self.db_path)
            raise
        finally:
            if conn is not None:
                conn.close()
    ...
    def read_industry_map_snapshot(self) -> dict[str, str]:
        """Read the current industry mapping without mutating schema state."""
        ...
        return {
            str(row["code"]): str(row["industry"] or "")
            for row in rows
...
def _default_ingestion_factory(**kwargs: Any) -> Any:
    manager_module = import_module("invest_evolution.market_data.manager")
    return manager_module.DataIngestionService(**kwargs)


class MarketDataGateway:
    """Unified external market-data sync boundary."""
    ...
    def __init__(
        ...
        *,
        ...
        self.ingestion_factory = ingestion_factory or _default_ingestion_factory
    ...
    def create_ingestion_service(self, *, repository: MarketDataRepository | None = None) -> Any:
    ...
    def ensure_runtime_derivatives(
        ...
        *,
        ...
        history_start = (
            datetime.strptime(start_date, "%Y%m%d") - timedelta(days=max(120, min_history_days * 2))
        ).strftime("%Y%m%d")
...
class DataQualityService:
    ...
    def _compute_audit_payload(self) -> dict[str, Any]:
        ...
        checks = {
            ...
            "index_date_range_valid": bool(index_date_range[0] and index_date_range[1])
    ...
    def audit(
        ...
        *,
        ...
        max_age_seconds: int = _QUALITY_AUDIT_MAX_AGE_SECONDS,
    ) -> dict[str, Any]:
    ...
    def __init__(
        ...
        **manager_kwargs: Any,
        ...
        if self.data_manager is None and self.repository is None:
            data_manager_cls = import_module("invest_evolution.market_data.manager").DataManager
...
class MarketQueryService:
    ...
    def __init__(self, dataset_service: Any | None = None, **dataset_kwargs: Any):
        from .datasets import WebDatasetService
