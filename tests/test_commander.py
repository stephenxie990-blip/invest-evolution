...
from invest_evolution.application.commander.bootstrap import acquire_runtime_lock
from invest_evolution.application.commander_main import PlaybookRegistry
...
def test_playbook_registry_templates(tmp_path: Path):
    registry = PlaybookRegistry(tmp_path)
    registry.ensure_default_playbooks()
...
def test_playbook_registry_parses_custom_files(tmp_path: Path):
    ...
    (tmp_path / "b.json").write_text(
        '{"id":"beta","name":"Beta Playbook","enabled":true,"priority":66,"description":"beta desc"}',
        encoding="utf-8",
    )
    ...
    alpha = next(playbook for playbook in playbooks if playbook.playbook_id == "alpha")
...
async def test_commander_runtime_init(tmp_path):
    ...
    assert runtime.cfg.playbook_dir.exists() is False
...
async def test_commander_runtime_init_is_read_only(tmp_path):
    ...
    assert not cfg.playbook_dir.exists()
...
async def test_execute_training_plan_passes_experiment_protocol_to_body(tmp_path, monkeypatch):
    ...
    assert observed["experiment_spec"]["manager_scope"]["allowed_manager_ids"] == ["momentum"]
...
def test_training_evaluation_summary_builds_promotion_verdict_against_baseline(tmp_path):
    ...
    (root_dir / 'leaderboard.json').write_text(json.dumps({
        ...
        'entries': [
            {'manager_id': 'momentum', 'manager_config_ref': 'momentum_v1', 'avg_return_pct': 0.4, 'avg_strategy_score': 0.45},
        ],
    }, ensure_ascii=False), encoding='utf-8')
    plan = runtime.create_training_plan(
        rounds=3,
        mock=True,
        manager_scope={'baseline_manager_ids': ['momentum']},
    ...
    assert summary['promotion']['candidate']['manager_id'] == 'value_quality'
    assert summary['promotion']['candidate']['manager_config_ref'] == 'value_quality_v1'
    assert summary['promotion']['baselines']['manager_ids'] == ['momentum']
    assert summary['promotion']['baselines']['entries'][0]['manager_id'] == 'momentum'
    ...
    summary = runtime._build_training_evaluation_summary(
        {
            ...
            'results': [
                {'status': 'ok', 'manager_id': 'momentum', 'manager_config_ref': 'momentum_v1', 'return_pct': 0.2, 'benchmark_passed': True, 'strategy_scores': {'overall_score': 0.55}},
            ],
        },
        plan=plan,
        run_id='run_reject',
...
def test_build_training_evaluation_summary_uses_helper_shape(tmp_path):
    ...
    payload = {
        "status": "completed",
        "results": [
            {"status": "ok", "manager_id": "momentum", "manager_config_ref": "momentum_v1", "return_pct": 1.2, "benchmark_passed": True, "strategy_scores": {"overall_score": 0.7}},
            {"status": "no_data"},
        ],
    }
...
def test_leaderboard_and_allocator_preview_do_not_mutate_snapshot(tmp_path):
    cfg = CommanderConfig(
        workspace=tmp_path / "workspace",
        playbook_dir=tmp_path / "strategies",
        state_file=tmp_path / "state" / "state.json",
    ...
    before = leaderboard_path.stat().st_mtime_ns

    runtime.get_leaderboard()
    runtime.get_allocator_preview(regime="bull", top_n=1, as_of_date="20260321")

    after = leaderboard_path.stat().st_mtime_ns
    assert after == before
...
def test_config_strategy_and_plugin_surfaces_expose_bounded_workflows(tmp_path):
    ...
    reloaded = runtime.reload_playbooks()
    ...
    assert reloaded["entrypoint"]["agent_kind"] == "bounded_playbook_agent"
    _assert_bounded_workflow_protocol(stock_strategies, domain="strategy", writes_state=False)
    _assert_bounded_workflow_protocol(reloaded, domain="playbook", writes_state=True)
...
def test_acquire_runtime_lock_does_not_remove_new_owner_after_stale_check(tmp_path: Path):
    ...
    def read_lock_payload() -> dict[str, Any]:
        ...
        lock_file.write_text(json.dumps(active_payload), encoding="utf-8")
        return dict(active_payload)

    def pid_alive(pid: int) -> bool:
        return pid == int(active_payload["pid"])
    ...
    with pytest.raises(RuntimeError, match="already active"):
        acquire_runtime_lock(
            ...
            read_lock_payload=read_lock_payload,
            pid_alive=pid_alive,
            os_module=os,
            socket_module=socket,
        )

    assert json.loads(lock_file.read_text(encoding="utf-8")) == active_payload
...
def test_reload_playbooks_resets_runtime_to_idle_and_persists_last_task(tmp_path):
    cfg = CommanderConfig(
        workspace=tmp_path / "workspace",
        playbook_dir=tmp_path / "strategies",
        state_file=tmp_path / "state" / "state.json",
    ...
    assert current_task is None
    assert last_task is not None
    assert last_task["type"] == "reload_playbooks"
    assert last_task["status"] == "ok"
    assert last_task["playbook_count"] == out["count"]
    assert payload["runtime"]["state"] == "idle"
    assert payload["runtime"]["last_task"]["playbook_count"] == out["count"]
