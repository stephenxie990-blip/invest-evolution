...
import invest_evolution.market_data.repository as market_repository_module
from invest_evolution.config import AgentConfigRegistry, IndustryRegistry
from invest_evolution.market_data import DataManager, DataSourceUnavailableError
from invest_evolution.market_data.datasets import T0DatasetBuilder
from invest_evolution.market_data.manager import DataIngestionService
from invest_evolution.market_data.manager import DataQualityService
...
def test_web_data_download_uses_unified_ingestion_service(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(web_server.get_ephemeral_web_state(), "data_download_running", False)
...
def test_market_data_cli_financials_requires_tushare_source(monkeypatch):
    import invest_evolution.market_data.manager as manager
...
def test_industry_registry_logs_warning_when_db_table_missing(tmp_path, caplog):
    db_path = tmp_path / "industry_missing_table.db"
    ...
    assert "Failed to load industry mappings from database" in caplog.text
    assert registry.get_industry("sh.600010") == "其他"


def test_industry_registry_logs_warning_when_override_is_invalid_json(tmp_path, caplog):
    db_path = tmp_path / "industry_ok.db"
    repo = MarketDataRepository(db_path)
    repo.initialize_schema()
    repo.upsert_security_master([
        {"code": "sh.600010", "name": "Foo", "list_date": "20200101", "industry": "银行", "source": "test"},
    ...
    override = tmp_path / "industry_map.json"
    override.write_text("{bad json", encoding="utf-8")

    with caplog.at_level("WARNING"):
        registry = IndustryRegistry(json_path=override, db_path=db_path)

    assert "Failed to load industry override mappings" in caplog.text
    assert registry.get_industry("sh.600010") == "银行"


def test_industry_registry_logs_warning_when_override_is_not_object(tmp_path, caplog):
    db_path = tmp_path / "industry_ok2.db"
    repo = MarketDataRepository(db_path)
    repo.initialize_schema()
    repo.upsert_security_master([
        {"code": "sh.600010", "name": "Foo", "list_date": "20200101", "industry": "银行", "source": "test"},
    ...
    override = tmp_path / "industry_map.json"
    override.write_text('["sh.600010"]', encoding="utf-8")

    with caplog.at_level("WARNING"):
        registry = IndustryRegistry(json_path=override, db_path=db_path)

    assert "Industry override mapping must be a JSON object" in caplog.text
    assert registry.get_industry("sh.600010") == "银行"


def test_agent_config_registry_ignores_non_object_payload(tmp_path, caplog):
    path = tmp_path / "agents_config.json"
    path.write_text('["bad"]', encoding="utf-8")

    with caplog.at_level("WARNING"):
        registry = AgentConfigRegistry(json_path=path)

    assert "Agent configs must be a JSON object" in caplog.text
    assert registry.all() == {}


def test_agent_config_registry_skips_non_object_entries(tmp_path, caplog):
    path = tmp_path / "agents_config.json"
    path.write_text(
        '{"hunter":{"llm_model":"fast"},"bad":"oops","":{"llm_model":"none"}}',
        encoding="utf-8",
    )
    ...
    assert "Agent config entry must be a JSON object" in caplog.text
    assert registry.get_config("hunter") == {"llm_model": "fast"}
    assert registry.get_config("bad") == {}
...
def test_check_training_readiness_marks_stale_offline_data_unready(tmp_path):
    db_path = tmp_path / "readiness_stale.db"
    repo = MarketDataRepository(db_path)
    repo.initialize_schema()
    repo.upsert_security_master([{"code": "sh.600001", "name": "Foo", "list_date": "20200101", "source": "test"}])
    bars = []
    ...
    manager = DataManager(db_path=str(db_path))
    diagnostics = manager.check_training_readiness("20240301", stock_count=1, min_history_days=20)

    assert diagnostics["ready"] is False
    assert diagnostics["stale_data"] is True
    assert diagnostics["requires_explicit_override"] is True
    assert any("早于训练截断日" in issue for issue in cast(list[str], diagnostics["issues"]))


def test_diagnose_training_data_marks_stale_offline_data_unready(tmp_path):
    db_path = tmp_path / "diagnose_stale.db"
    repo = MarketDataRepository(db_path)
    repo.initialize_schema()
    repo.upsert_security_master([{"code": "sh.600001", "name": "Foo", "list_date": "20200101", "source": "test"}])
    bars = []
    ...
    manager = DataManager(db_path=str(db_path))
    diagnostics = manager.diagnose_training_data("20240301", stock_count=1, min_history_days=20)
...
def test_load_stock_data_uses_lightweight_training_readiness(tmp_path, monkeypatch):
    ...
    repo.upsert_security_master([{"code": "sh.600001", "name": "Foo", "list_date": "20200101", "source": "test"}])
    bars = []
    for trade_date in pd.date_range("2023-09-01", periods=100, freq="B"):
        bars.append({
            "code": "sh.600001",
            "trade_date": trade_date.strftime("%Y%m%d"),
            "open": 10,
...
def test_load_stock_data_does_not_use_stale_offline_data_when_online_fallback_disabled(tmp_path):
    db_path = tmp_path / "load_stale_blocked.db"
    repo = MarketDataRepository(db_path)
    repo.initialize_schema()
    repo.upsert_security_master([{"code": "sh.600001", "name": "Foo", "list_date": "20200101", "source": "test"}])
    bars = []
    ...
    manager = DataManager(db_path=str(db_path))
    manager._allow_online_fallback = False
    manager._online = None

    with pytest.raises(DataSourceUnavailableError) as exc_info:
        manager.load_stock_data("20240301", stock_count=1, min_history_days=20, include_future_days=0)

    payload = exc_info.value.to_dict()
    assert isinstance(payload.get("offline_diagnostics"), dict)
    assert payload["offline_diagnostics"]["ready"] is False
    assert payload["offline_diagnostics"]["stale_data"] is True
...
def test_repository_connect_raises_and_logs_on_connect_failure(tmp_path, monkeypatch, caplog):
    repo = MarketDataRepository(tmp_path / "connect_fail.db")

    def _raise_connect(*args, **kwargs):
        raise sqlite3.OperationalError("connect boom")

    monkeypatch.setattr(market_repository_module.sqlite3, "connect", _raise_connect)
    with caplog.at_level("ERROR"):
        with pytest.raises(sqlite3.OperationalError, match="connect boom"):
            with repo.connect():
                pass
    assert "Database operation failed" in caplog.text


def test_repository_connect_rolls_back_on_sqlite_error(tmp_path):
    repo = MarketDataRepository(tmp_path / "rollback.db")
    repo.initialize_schema()

    with pytest.raises(sqlite3.OperationalError):
        with repo.connect() as conn:
            conn.execute(
                "INSERT INTO security_master(code, name, list_date, source) VALUES (?, ?, ?, ?)",
                ("sh.600001", "Foo", "20200101", "test"),
            )
            conn.execute("INSERT INTO not_exists(value) VALUES (1)")

    with repo.connect() as conn:
        count = conn.execute("SELECT COUNT(*) FROM security_master WHERE code = ?", ("sh.600001",)).fetchone()[0]
    assert count == 0


def test_repository_connect_falls_back_to_delete_when_wal_setup_fails(tmp_path, monkeypatch, caplog):
    repo = MarketDataRepository(tmp_path / "wal_fallback.db")
    ...
    class FakeConnection:
        ...
        def execute(self, statement, *args, **kwargs):
            ...
            if statement == "PRAGMA journal_mode=WAL;":
                raise sqlite3.OperationalError("wal blocked")
            return self
    ...
    monkeypatch.setattr(
        market_repository_module.sqlite3,
        "connect",
    ...
    assert "Falling back to DELETE journal mode" in caplog.text
