...
import invest_evolution.application.commander.status as status_module
...
from invest_evolution.market_data.datasets import WebDatasetService
...
async def test_commander_runtime_lock_lifecycle(tmp_path: Path):
    ...
    assert status["runtime"]["state_source"] == "live_runtime"
    assert status["runtime"]["live_runtime"] is True
    await rt.stop()
...
def test_collect_data_status_logs_failure(monkeypatch, caplog):
    class DummyQueryService:
        def get_status_summary(self, *, refresh: bool = False):
            raise RuntimeError(f"refresh={refresh}")
    ...
    assert payload["detail_mode"] == "slow"
    assert "Failed to collect data status for detail_mode=slow" in caplog.text


def test_brain_governance_metrics_logs_failure(caplog):
    class Brain:
        def _governance_metrics_snapshot(self):
            raise RuntimeError("boom")

    runtime = type("Runtime", (), {"brain": Brain()})()

    with caplog.at_level("WARNING"):
        payload = status_module._brain_governance_metrics(runtime)

    assert payload == {}
    assert "Failed to collect brain governance metrics" in caplog.text
...
def test_commander_config_derives_runtime_artifact_paths(tmp_path: Path):
    ...
    assert cfg.artifact_log_dir == tmp_path / "state" / "artifacts"
...
def test_train_controller_accepts_injected_artifact_paths(tmp_path: Path):
    ...
    meeting_dir = tmp_path / "artifacts"
    ...
    assert controller.artifact_recorder.base_dir == meeting_dir
