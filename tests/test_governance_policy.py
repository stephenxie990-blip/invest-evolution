...
def test_evaluate_optimization_event_contract_requires_cycle_id_and_lineage():
    payload = {
        ...
        "status": "ok",
        "decision": {"runtime_config_ref": "candidate.yaml"},
        ...
        "lineage": build_optimization_event_lineage(
            ...
            candidate_runtime_config_ref="candidate.yaml",
...
def test_evaluate_governance_quality_gate_blocks_negative_score_candidate_stage():
...
def test_evaluate_promotion_discipline_marks_override_and_candidate_states():
    ...
    assert infer_deployment_stage(
        run_context={"candidate_runtime_config_ref": "configs/candidate.yaml", "promotion_decision": {"applied_to_active": False}}
    )["deployment_stage"] == "candidate"
...
def test_evaluate_promotion_discipline_ignores_pruned_history_when_counting_pending_candidates():
    discipline = evaluate_promotion_discipline(
        run_context={
            "active_runtime_config_ref": "configs/active.yaml",
            "candidate_runtime_config_ref": "configs/candidate_new.yaml",
            "promotion_decision": {"applied_to_active": False},
        },
        cycle_history=[
            {
                "lineage_record": {
                    ...
                    "candidate_runtime_config_ref": "configs/candidate_old.yaml",
                }
            }
        ],
        policy={"max_pending_candidates": 1},
        optimization_events=[],
    )

    assert discipline["pending_candidate_count"] == 1
    assert discipline["status"] == "candidate_pending"


def test_evaluate_promotion_discipline_ignores_shadow_history_when_counting_pending_age():
    discipline = evaluate_promotion_discipline(
        run_context={
            "active_runtime_config_ref": "configs/active.yaml",
            "candidate_runtime_config_ref": "configs/candidate.yaml",
        ...
        cycle_history=[
            {
                ...
                "validation_report": {"shadow_mode": True},
            },
            {
                "lineage_record": {
                    "deployment_stage": "candidate",
        ...
        policy={"max_pending_cycles": 3},
        optimization_events=[],
    )

    assert discipline["pending_candidate_age"] == 1
    assert discipline["status"] == "candidate_pending"


def test_normalize_config_ref_rejects_out_of_root_paths():
    assert normalize_config_ref("/tmp/escape.yaml") == ""
    assert normalize_config_ref("../../escape.yaml") == ""
