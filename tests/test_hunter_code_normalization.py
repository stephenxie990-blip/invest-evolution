...
@pytest.mark.parametrize("agent_factory", [TrendHunterAgent, ContrarianAgent])
def test_hunters_fallback_handles_sparse_candidates(agent_factory):
    agent = agent_factory(llm_caller=False)

    result = agent._fallback_analysis([{"code": "sh.600058", "rsi": "not-a-number"}])

    assert result["contract_status"] == "fallback_algorithm"
    assert result["picks"][0]["code"] == "sh.600058"
    assert isinstance(result["picks"][0]["reasoning"], str)
    assert result["picks"][0]["reasoning"]
