...
from invest_evolution.application.training.observability import build_freeze_report
from invest_evolution.application.training.observability import SelfAssessmentSnapshot
    ...
    controller.cycle_history = [
        TrainingResult(
            ...
            lineage_record={
                ...
                "active_runtime_config_ref": "active.yaml",
                "candidate_runtime_config_ref": "candidate.yaml" if index % 3 == 0 else "",
            },
        )
...
def test_feedback_optimization_plan_skips_legacy_overall_fallback_scope(tmp_path):
    controller = SelfLearningController(
        output_dir=str(tmp_path / "training"),
        artifact_log_dir=str(tmp_path / "artifacts"),
        config_audit_log_path=str(tmp_path / "audit" / "changes.jsonl"),
    ...
    plan = controller._build_feedback_optimization_plan(  # pylint: disable=protected-access
        {
            **_make_feedback(bias="tighten_risk", sample_count=200),
            "scope": {
