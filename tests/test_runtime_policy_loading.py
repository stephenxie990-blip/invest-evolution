...
def test_controller_loads_runtime_policy_from_manager_runtime_config(tmp_path):
    ...
    assert controller.selection_agent_weights["trend_hunter"] == 1.0
...
def test_manager_runtime_config_exposes_summary_scoring(tmp_path):
    controller = SelfLearningController(
        output_dir=str(tmp_path / "out"),
        artifact_log_dir=str(tmp_path / "artifacts"),
        config_audit_log_path=str(tmp_path / "state" / "audit.jsonl"),
