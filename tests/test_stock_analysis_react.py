...
from invest_evolution.application.stock_analysis import StockAnalysisService, StockAnalysisStrategyStore
...
def test_stock_analysis_lists_seeded_default_strategies(tmp_path: Path):
    service = _build_service(tmp_path)

    items = service.list_strategies()
    names = {item["name"] for item in items}

    assert {"chan_theory", "trend_following"} <= names
...
def test_stock_strategy_scoring_coerce_returns_empty_on_non_mapping():
    assert StockAnalysisStrategyStore._coerce_scoring(["bad"]) == {}


def test_stock_strategy_scoring_coerce_skips_invalid_weights(caplog):
    with caplog.at_level("WARNING"):
        scoring = StockAnalysisStrategyStore._coerce_scoring(
            {
                "trend": "1.2",
                "risk": "bad",
                "momentum": 0.8,
    ...
    assert scoring == {"trend": 1.2, "momentum": 0.8}
    assert "Invalid strategy scoring weight" in caplog.text
