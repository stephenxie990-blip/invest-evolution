...
from invest_evolution.interfaces.web.runtime import InProcessRuntimeFacade
...
def _install_runtime(monkeypatch, runtime):
    loop = object()
    monkeypatch.setattr(web_server, '_runtime_facade', InProcessRuntimeFacade(
...
def test_api_status_returns_bounded_workflow_contract_headers(client_with_runtime):
...
def test_api_status_supports_human_text_contract(client_with_runtime):
    client, _runtime = client_with_runtime
...
def test_api_events_summary_emits_contract_headers(client_with_runtime):
    ...
    append_event_row(
        ...
        {'dominant_manager_id': 'momentum', 'active_manager_ids': ['momentum'], 'reasoning': 'test'},
        source='runtime',
    )
...
def test_api_status_fast_detail_emits_contract_headers_for_snapshot_payload(client_with_runtime):
    client, _runtime = client_with_runtime

    res = client.get('/api/status?detail=fast')
...
def test_api_data_status_returns_bounded_workflow_contract_headers(client_with_runtime):
    client, _runtime = client_with_runtime
...
def test_api_data_status_supports_human_text_contract(client_with_runtime):
    client, _runtime = client_with_runtime
...
def test_api_managers_route_is_removed_from_public_api_surface(client_with_runtime):
    client, _runtime = client_with_runtime

    res = client.get('/api/managers?view=human')
...
def test_api_playbooks_route_is_removed_from_public_api_surface(client_with_runtime):
    client, _runtime = client_with_runtime

    res = client.get('/api/playbooks?view=human')

    assert res.status_code == 404


def test_removed_strategies_alias_route_returns_404(client_with_runtime):
    client, _runtime = client_with_runtime
...
def test_api_playbooks_reload_route_is_removed_and_legacy_alias_returns_404(
    client_with_runtime,
):
    client, _runtime = client_with_runtime

    res = client.post('/api/playbooks/reload')
    assert res.status_code == 404

    legacy = client.post('/api/strategies/reload')
    assert legacy.status_code == 404


def test_api_cron_routes_are_removed_from_public_api_surface(client_with_runtime):
    client, runtime = client_with_runtime
...
def test_api_training_plan_execute_returns_training_workflow_contract_headers(
    client_with_runtime, monkeypatch
):
    client, runtime = client_with_runtime

    async def fake_execute_training_plan(plan_id: str):
        payload = runtime._build_confirmation_required_workflow(  # type: ignore[attr-defined]
            domain="training",
    ...
    runtime.execute_training_plan = fake_execute_training_plan

    res = client.post('/api/lab/training/plans/plan_demo/execute')

    assert res.status_code == 200
    payload = res.get_json()
    assert payload['status'] == 'confirmation_required'
    assert payload['pending'] == {'rounds': 2, 'mock': False}
    _assert_contract_headers(res, domain='training', operation='execute_training_plan')


def test_api_training_plan_execute_view_human_falls_back_to_json_contract(
    client_with_runtime, monkeypatch
):
    client, runtime = client_with_runtime
    ...
    res = client.post('/api/lab/training/plans/plan_demo/execute?view=human')
...
def test_api_data_download_returns_confirmation_contract_headers(client_with_runtime):
    client, _runtime = client_with_runtime
...
def test_api_data_download_returns_bounded_workflow_contract_headers_when_confirmed(
    client_with_runtime, monkeypatch
):
    client, runtime = client_with_runtime
...
def test_api_chat_falls_back_for_non_json_reply(client_with_runtime):
    client, runtime = client_with_runtime
    ...
    assert res.status_code == 200
    payload = res.get_json()
    assert payload['reply'] == 'plain-text-reply'
    assert payload['message'] == 'plain-text-reply'
    assert payload['session_key'].startswith('api:chat:')
    assert payload['chat_id'].startswith('chat:')
...
def test_api_status_rejects_unknown_detail(client_with_runtime):
    client, _runtime = client_with_runtime
    res = client.get('/api/status?detail=xml')
    assert res.status_code == 400
    assert 'detail must be one of' in res.get_json()['error']
