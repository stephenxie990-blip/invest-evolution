...
def test_memory_api_routes_are_removed_from_public_api_surface(tmp_path, monkeypatch):
    runtime = _make_runtime(tmp_path)
    ...
    selection_json = tmp_path / "artifacts" / "selection" / "selection_0001.json"
    ...
    selection_md = tmp_path / "artifacts" / "selection" / "selection_0001.md"
    selection_md.write_text("# 选股工件\n- 000001.SZ", encoding="utf-8")

    review_json = tmp_path / "artifacts" / "manager_review" / "manager_review_0001.json"
    review_json.parent.mkdir(parents=True, exist_ok=True)
    review_json.write_text(json.dumps({"report": {"verdict": "continue"}}, ensure_ascii=False), encoding="utf-8")

    review_md = tmp_path / "artifacts" / "manager_review" / "manager_review_0001.md"
    review_md.write_text("# 经理复盘工件\n保持纪律", encoding="utf-8")

    allocation_json = tmp_path / "artifacts" / "allocation_review" / "allocation_review_0001.json"
    allocation_json.parent.mkdir(parents=True, exist_ok=True)
    allocation_json.write_text(json.dumps({"report": {"verdict": "continue"}}, ensure_ascii=False), encoding="utf-8")

    allocation_md = tmp_path / "artifacts" / "allocation_review" / "allocation_review_0001.md"
    allocation_md.write_text("# 组合复盘工件\n继续观察", encoding="utf-8")
            ...
            "results": [
                {
                    ...
                    "artifacts": {
                        ...
                        "selection_artifact_json_path": str(selection_json),
                        "selection_artifact_markdown_path": str(selection_md),
                        "manager_review_artifact_json_path": str(review_json),
                        "manager_review_artifact_markdown_path": str(review_md),
                        "allocation_review_artifact_json_path": str(allocation_json),
                        "allocation_review_artifact_markdown_path": str(allocation_md),
    ...
    assert list_human.status_code == 404

    detail_res = client.get(f"/api/memory/{rec.id}")
    assert detail_res.status_code == 404

    detail_human = client.get(f"/api/memory/{rec.id}?view=human")
    assert detail_human.status_code == 404


def test_memory_api_zero_limit_route_is_removed_from_public_api_surface(tmp_path, monkeypatch):
    runtime = _make_runtime(tmp_path)
    runtime.memory.append(
        kind="note",
        session_key="runtime:test",
        content="hello",
        metadata={},
    )
    ...
    res = client.get("/api/memory?limit=0")
