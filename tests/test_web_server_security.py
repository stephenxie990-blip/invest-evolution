...
def test_bootstrap_runtime_services_starts_runtime_once(monkeypatch):
    ...
    monkeypatch.setattr(web_server, 'ensure_environment', lambda **kwargs: env_calls.append(kwargs))
...
def test_bootstrap_runtime_services_logs_context_on_start_failure(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.DEBUG):
        with pytest.raises(RuntimeError, match='runtime start failed'):
            web_server.bootstrap_runtime_services(host='127.0.0.1', mock=True, source='cli')

    assert len(callbacks) == 1
    assert 'Commander runtime bootstrap failed: host=127.0.0.1 mock=True source=cli runtime_type=FakeRuntime loop_thread=web-event-loop:cli' in caplog.text
    assert 'Failed to stop event loop after bootstrap error: host=127.0.0.1 mock=True source=cli loop_type=FakeLoop' in caplog.text
...
def test_embedded_runtime_bootstrap_is_disabled_by_default(monkeypatch):
    ...
    monkeypatch.setattr(web_server, 'bootstrap_runtime_services', lambda **kwargs: called.append(kwargs))

    result = web_server.bootstrap_embedded_runtime_if_enabled(host='127.0.0.1', source='gunicorn')
...
def test_embedded_runtime_bootstrap_rejects_multi_worker_mode(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match='requires GUNICORN_WORKERS=1'):
        web_server.bootstrap_embedded_runtime_if_enabled(host='127.0.0.1', source='gunicorn')
...
def test_embedded_runtime_bootstrap_uses_explicit_helper(monkeypatch):
    ...
    result = web_server.bootstrap_embedded_runtime_if_enabled(host='127.0.0.1', mock=True, source='gunicorn')
...
def test_shutdown_runtime_services_logs_context_on_stop_failure(monkeypatch, caplog):
    ...
    monkeypatch.setattr(web_server, '_loop', FakeLoop())
    monkeypatch.setattr(web_server.asyncio, 'run_coroutine_threadsafe', lambda coro, loop: (coro.close(), loop, FakeFuture())[2])

    with caplog.at_level(logging.DEBUG):
        web_server.shutdown_runtime_services()

    assert 'Failed to stop commander runtime cleanly during shutdown: runtime_type=FakeRuntime loop_type=FakeLoop' in caplog.text
    assert 'Failed to stop web event loop cleanly during shutdown: runtime_type=FakeRuntime loop_type=FakeLoop' in caplog.text
    assert web_server._runtime is None
    assert web_server._loop is None
...
def test_wsgi_import_is_side_effect_free(monkeypatch):
    ...
    monkeypatch.setattr(app_web_server, 'bootstrap_runtime_services', _unexpected_bootstrap)
